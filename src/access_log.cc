...
//
#if USE_SHAGA
if(Config.Shaga.enable_shaga){
//
snprintf(ShagaAccess.access_timestamp,13,"%9ld.%03d",
		 (long int)current_time.tv_sec,( (int)current_time.tv_usec / 1000));
sscanf((user ? user : dash_str),"%s",ShagaAccess.access_user);
sscanf(client,"%s",ShagaAccess.access_ip);
snprintf(ShagaAccess.access_byte,15,"%ld",(long int)al->cache.size);
//
	writeToDB(ShagaAccess.access_timestamp,
			ShagaAccess.access_user,
			ShagaAccess.access_ip,
			ShagaAccess.access_byte); /* write to db TIMESTAMP ,USER ,IP ,BYTE */
}
#endif
//
