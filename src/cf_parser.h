...
default_all(void)
{
	...
	default_line("cache_log /home/seagull/squid3_test/var/logs/cache.log");
	default_line("cache_store_log /home/seagull/squid3_test/var/logs/store.log");
	...
	default_line("mime_table /home/seagull/squid3_test/etc/mime.conf");
...
#endif
	default_line("pid_filename /home/seagull/squid3_test/var/logs/squid.pid");
	...
	default_line("diskd_program /home/seagull/squid3_test/libexec/diskd");
#if USE_UNLINKD
	default_line("unlinkd_program /home/seagull/squid3_test/libexec/unlinkd");
#endif
#if USE_ICMP
	default_line("pinger_program /home/seagull/squid3_test/libexec/pinger");
#endif
	...
	default_line("icon_directory /home/seagull/squid3_test/share/icons");
	default_line("global_internal_static on");
	default_line("short_icon_urls on");
	default_line("error_directory /home/seagull/squid3_test/share/errors/English");
...
#if USE_DNSSERVERS
	default_line("cache_dns_program /home/seagull/squid3_test/libexec/dnsserver");
#endif
#if USE_DNSSERVERS
...
#endif
#if USE_SHAGA
	default_line("enable_db_config off");
#endif
	cfg_filename = NULL;
}
...
defaults_if_none(void)
{
	...
	if (check_null_cachedir(Config.cacheSwap)) {
		default_line("cache_dir ufs /home/seagull/squid3_test/var/cache 100 16 256");
	}
#if USE_WCCPv2
...
parse_line(char *buff)
{
	...
	if (!strcmp(token, "debug_options")) {
		parse_debug(&Config.debugOptions);
		return 1;
	};
...
#if USE_SHAGA
	if (!strcmp(token, "enable_shaga_engine")) {
		parse_onoff(&Config.Shaga.enable_shaga);
		return 1;
	};
#endif
#if USE_SHAGA
	if (!strcmp(token, "shaga_db_host")) {
		parse_string(&Config.Shaga.db_host);
		return 1;
	};
#endif
#if USE_SHAGA
	if (!strcmp(token, "shaga_db_port")) {
		parse_ushort(&Config.Shaga.db_port);
		return 1;
	};
#endif
#if USE_SHAGA
	if (!strcmp(token, "shaga_db_name")) {
		parse_string(&Config.Shaga.db_name);
		return 1;
	};
#endif
#if USE_SHAGA
	if (!strcmp(token, "shaga_db_user")) {
		parse_string(&Config.Shaga.db_user);
		return 1;
	};
#endif
#if USE_SHAGA
	if (!strcmp(token, "shaga_db_pwd")) {
		parse_string(&Config.Shaga.db_pwd);
		return 1;
	};
#endif
#if USE_SHAGA
	if (!strcmp(token, "enable_db_config")) {
		parse_onoff(&Config.Shaga.enable_db_config);
		return 1;
	};
#endif
	return 0; /* failure */
...
dump_config(StoreEntry *entry)
{
	...
	dump_debug(entry, "debug_options", Config.debugOptions);
...
#if USE_SHAGA
	dump_onoff(entry, "enable_shaga_engine", Config.Shaga.enable_shaga);
#endif
#if USE_SHAGA
...
#endif
#if USE_SHAGA
	dump_onoff(entry, "enable_db_config", Config.Shaga.enable_db_config);
#endif
}
...
free_all(void)
{
	...
	free_debug(&Config.debugOptions);
...
#if USE_SHAGA
	free_onoff(&Config.Shaga.enable_shaga);
#endif
#if USE_SHAGA
...
#endif
#if USE_SHAGA
	free_onoff(&Config.Shaga.enable_db_config);
#endif
}
